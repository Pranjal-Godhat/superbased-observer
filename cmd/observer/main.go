...
func buildWatcherWithOverride(ctx context.Context, configPath, adapterFilter string) (*watcher.Watcher, func(), error) {
	...
	if cfg.Observer.Retention.PruneOnStartup {
		if res, err := runRetention(ctx, cfg, database); err != nil {
			...
			if res.SizeCapUnmet {
				logger.Warn("retention: DB still over max_db_size_mb after shedding aged actions — the bulk is in other tables (token_usage / cache_*). Recent actions were protected by the keep-floor. Raise [observer.retention].max_db_size_mb or add token retention.",
