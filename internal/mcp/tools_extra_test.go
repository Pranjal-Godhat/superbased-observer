...
// Smoke test: tools/list now returns 18 tools (12 spec §11.2 + the
...
func TestServer_ToolsListReturnsEighteen(t *testing.T) {
	s, _, _ := testServer(t)
	resp := rpcCall(t, s, "tools/list", 1, nil)
	tools := resp["result"].(map[string]any)["tools"].([]any)
	if len(tools) != 18 {
		t.Errorf("tools count: %d (want 18)", len(tools))
	}
}
