...
- **Output Composition (verbosity) — code vs. explanation, by language, with
  ...
  CLI, `GET /api/session/<id>/verbosity` + a SessionDetailPanel card, `GET
  /api/verbosity/aggregate` + an Analysis Output-composition section, and the
...
- **Gemini conversation compressor.** A Google `generateContent` provider lane
  in the proxy (`providerForPath` → `parseGeminiResponse` / `parseGeminiStream`,
...
- **Admin-configurable message-content retention** (`[dashboard.content_retention]
  .otel_content_days`, default 0 = keep forever): a daily server-side sweep
...
- **`scripts/restart-daemon.sh`** — a proxy-safe, atomic daemon-restart
  helper (route off → stop → relaunch → route on) with `--compression
  on|off`. Killing the daemon while a client routes through the proxy
...
- **Conversation compression no longer corrupts large requests.** The
  ...
  the request into invalid JSON and triggering an upstream `400 unexpected
  character` on proxied Claude Code once a conversation grew past ~214KB.
...
### Added — the Security and Routing pages became operable surfaces (the guard/routing usability arc)

The guard layer and model routing merged after the main usability arc
closed, so neither got its treatment: both pages were read-only
viewers and every routine operation meant hand-editing config and
hunting CLI flags. The arc (phases G1, R1, G2, R2, X3) applies the
same principles — dashboard controls for routine operations,
evidence-bearing consent dialogs, honest restart semantics, one
config-write seam, help-drawer coverage — to both:

- **Security**: a consent-gated mode control that shows the simulate
  evidence before enforce; the enforce-readiness card (a replay of
  real history under the enforce projection with per-rule blocking
  concentration); the approvals register UI (scoped, expiring, live
  immediately); event drill-down filters + top-rules rollup +
  session deep-links; effective-rules resolution (custom layers show
  their source); the MCP pin inventory with approve; a lint-gated
  guard-policy editor (422 with problems on malformed input, `.bak`
  swap-undo); compliance evidence buttons (report / export /
  verify-audit as jobs with downloads); budget guardrails suggested
  from observed spend with a daily burn-down meter; and the
  observe→enforce migration runbook (docs/guard-enforce-runbook.md).
- **Routing**: counterfactual "Preview savings" while routing is off;
  a `routing` Settings section (adoption-funnel knobs only; complex
  shapes and secrets stay file-only and survive every save); the
  advise-shadow readiness ladder with a consent-gated promote
  control; per-decision drill-down with plain-language reason codes;
  Channel A apply from the dashboard (dry-run preview with §R7.2
  parity evidence, one consent per file write, backups, per-file and
  unified revert, and an append-only audit ledger — `routing apply
  --history` is the CLI view); a lint-gated `[[routing.rules]]`
  editor riding the one config seam; calibration-demotion visibility
  (demoted rules marked in the rule table, honest about in-memory
  state); the first-30-days adoption runbook and a 9-recipe rules
  gallery (docs/model-routing.md), every recipe machine-verified
  through the real lint gate.
- **Advisor**: two navigate-only posture nudges close the loop —
  `guard_observe_idle` (recurring high-severity verdicts, zero
  tuning → review Security) and `routing_evidence_ready` (the shadow
  gate met, routing not promoted → review the Shadow card). Detector
  registry 18 → 20.

Help registry 240 → 254. Privacy invariants held throughout: no
remote enforce toggle exists for either feature; routing decision
rows, calibration, and the apply ledger are node-local.

### Added — help coverage for the Security / Routing / Suggestions pages; guard rule IDs resolve to their definitions

The three newest dashboard pages shipped with almost no links into the
help drawer (Security and Routing had zero; Suggestions had two),
while established pages carry one on nearly every element. The help
registry grows 217 → 240 entries: page-tab entries for Security and
Routing, a tile entry for every hero stat (guard posture, verdict
counts, audit chain; routing decisions, realized / would-have savings,
calibration cells; avoidable spend, open suggestions, sessions
scanned), a section entry for every table/board (verdict timeline,
enforcement coverage; advise shadow, policy rule table, decisions
feed, savings by tier, tier map, model health), and glossary entries
for the guard rule catalog, the allow/flag/ask/deny decision
vocabulary, and the anatomy of a suggestion card. Each entry states
what the element means, how it is calculated (formula + API source
where applicable), and cross-links its relatives. The stale
Suggestions tab entry ("Phase 1 ships four detectors") was rewritten
for the current 18-detector engine.

Guard verdicts no longer show bare rule IDs: a new /api/guard/rules
endpoint serves the built-in rule catalog (policy.Catalog — id,
category, severity, observe/enforce decision pair, doc one-liner, and
the remediation advice each rule carries), and the Security page's
verdict timeline renders the rule's actual definition with the ID as
the secondary line, full breakdown in a hover tooltip. Rules from
user/project/org policy layers (or an older daemon) degrade to the
bare ID as before.

### Changed — Routing page restyled to the dashboard design system; sidebar badges made uniform

The Routing page (merged with the model-routing PR) shipped with raw
inline-styled markup that didn't match the dashboard's design system —
plain text tables, no card surfaces, dead CSS classes. It now uses the
house primitives end to end (HeroStat tiles, card sections, the shared
table/pill/segmented-control patterns — mirroring the Security page
from the same arc) and honors the global TopBar window filter for its
windowed queries (savings / decisions feed / advise shadow; the
backend day clamps widened from 365 to the sibling pages' 36500 so
"All" means all-time). Sidebar nav badges are now uniform across data
pages: Live (sessions active in the last 15 minutes), Security (guard
verdicts), Search (FTS5-indexed excerpts — the searchable corpus), and
Routing (router decisions) join the existing badges; Overview and the
Configure group stay deliberately un-badged. The new counts ride
/api/status (`live_sessions` / `guard_events` / `router_decisions`)
and degrade gracefully against older daemons that don't serve them.

### Added — per-tool compression profiles now engage for hookless clients (header-signature tool resolution)

The compression profile router's per-tool tier (`[profiles.by_tool]`,
`observer profile assign tool:<name> …`, and `tool:` experiment
classes) previously engaged only for clients that feed the session
pid bridge — in practice just Claude Code, whose SessionStart hook is
the bridge's lone feeder. Every hookless client resolved `tool=""`
at the proxy and silently fell through to its per-provider profile
(deviation D20). The proxy now carries a header-signature fallback
(`internal/proxy/toolheaders.go`): on a bridge miss, an ordered table
of verified request-header signatures resolves the tool — Codex via
its `codex_cli_rs` User-Agent/`Originator` header, Kilo CLI via
`X-Title: Kilo Code` / `Kilo-Code/` UA, opencode via `X-Title:
opencode` / `opencode/` UA, and `claude-cli/` UA as a no-hooks safety
net. Hook-fed bridge identity always wins when present; a request
matching no signature degrades to the per-provider tier exactly as
before. Cline CLI sends only stock SDK headers (verified — nothing
distinctive to match) and intentionally has no row; it keeps its
per-provider profile.

### Fixed — cline-cli and hermes token double-counting (cross-source emission)

Every Cline CLI session's tokens and cost surfaced at **exactly 2×**
(operator-reported): the adapter emitted BOTH the session-level usage
aggregate (from `sessions.metadata_json.usage`) AND per-message
metrics rows (from `<id>.messages.json`), and nothing downstream
could merge them — the two shapes differ on both halves of the
store's `UNIQUE(source_file, source_event_id)` key, and the cost
engine's three-level dedup only pairs proxy↔jsonl rows. The aggregate
is now a fallback, emitted only for terminal-status sessions whose
messages.json carries no per-message `metrics` blocks (pre-metrics
cline builds).

The same defect class was confirmed live on hermes (the 2026-06-05
audit's H1 finding, previously documented as open): a hook-covered
session carried both the hook's per-API-call token rows and the
SQLite session aggregate, plus a `run_command` action from both
paths. The hermes watcher path now consults a `SessionHookChecker`
(mirroring cursor/clinecli) and, for hook-covered sessions,
suppresses the event classes the hook also emits (tool_calls + the
token aggregate) while keeping the SQLite-only classes (user_prompt,
assistant text, system_prompt). Also wired the clinecli adapter's
existing `SessionHookChecker` at the cmd assembly — the seam shipped
in v1.8.2 but was never injected, so cline-hook installs would have
double-counted the moment hook commands were registered.

Existing inflated rows in a node's DB are repairable with a targeted
delete of the redundant aggregates (`source_event_id LIKE
'tk:%:session_usage'` where per-message rows exist for the session);
new scans emit correctly.

### Changed — codex-safe profile gains the tools-defs trim (A6-gated)

The `codex-safe` compression profile's `compress_types` grows from
`["logs"]` to `["logs","tools"]`: tool **definitions** in the OpenAI
request envelope are trimmed (descriptions of 3+ paragraphs keep the
first 2; JSON-Schema `examples` stripped — names, types,
required-ness, enums never touched; tool_result bodies never
touched). A/B-gated per the profile content-refresh plan: run A6
(2026-06-11, hash-split through the productized experiments feature,
n=7 control / n=9 candidate, salted bug-fix tasks, gpt-5.4 through
the codex CLI) showed the trim firing on **117/117** candidate
requests (~0.8KB of definition prose per request, deterministic and
byte-stable so OpenAI's implicit prompt cache keeps hitting —
per-session `implicit_hit` was unchanged-to-better), with full
task-completion parity and ±0% turns. Honest sizing: on a lean
envelope (no MCP servers registered) the saving is sub-1% of session
cost — cost means could not distinguish the arms (+1.7%, 0.2 SE at
CV 18%); adoption rests on the deterministic positive trim, the
clean no-harm profile, and scaling — the trim grows with envelope
bulk, so MCP-registered codex installs save substantially more
(`codex-variant`, which has shipped the same trim since v1.7.23,
measured −10% on `-codex` models). Opt out by pinning
`compress_types = ["logs"]` in a custom profile. Full record:
`docs/plans/profile-content-refresh-ab-plan-2026-06-10.md` §8.

### Added — dashboard setup, health, and onboarding surfaces (usability arc)

A six-phase usability pass (2026-06-10 → 2026-06-11; plan + tracker at
`docs/plans/usability-review-2026-06-10.md`) rebuilt the dashboard
around one principle: every setup step the dashboard could previously
only *describe*, it can now *do* — with a preview of the exact file
edit and an explicit consent click before every write, and honest
status everywhere else. By surface:

- **One-click durable proxy routing** for Claude Code and Codex from
  the Compression tab's proxy banner (`POST /api/setup/claude` /
  `/api/setup/codex`): preview → consent → write, byte-equivalent to
  what `observer init` writes, with a 409-force path when the target
  config drifted from the preview.
- **Setup wizard + connected-tools panel.** A per-tool wizard (hooks /
  MCP / proxy route, one write per consent, `POST /api/setup/hooks|mcp`)
  on top of a read-only connected-tools matrix that probes each
  detected AI tool's hook/MCP/route state without touching anything.
  The MCP step is never pre-selected and carries the ~1,800-token
  per-turn schema-overhead note up front; a codex hook-trust card
  explains Codex's notify-command trust prompt.
- **Launch buttons** (dashboard + VS Code extension): open a detected
  AI tool from the connected-tools panel via a hardcoded-argv
  allow-list, with an honest `{spawned, command}` contract — the UI
  shows exactly what was executed and claims nothing beyond "the
  process started".
- **Doctor / health panel + failures surface.** The `observer doctor`
  checks (now 13) render in Settings → Health, and recent command
  failures group per command with session deep-links.
- **"Proxy routing gap" diagnostic.** Live verification produced the
  finding behind it: an AI tool durably routed at the proxy while the
  daemon is down *hangs retrying* — no bypass, no fast fail. Doctor
  and the Health panel now WARN when a tool is routed and capture is
  landing but no API traffic has arrived through the proxy in the same
  window. The restart-pending banner stays banner-only by explicit
  decision (no self-respawn).
- **MCP / org-share / OTel config sections + MCP value meter.** The
  remaining config groups become editable dashboard sections (the
  org-share section deliberately cannot touch enrolment identity), and
  the MCP meter weighs the per-turn schema overhead against the MCP
  calls actually observed on the node — it says "unused" when that is
  the truth.
- **Onboarding + first-run.** An empty Overview shows an onboarding
  checklist; a one-time toast fires on the first captured action;
  milestone cards ($10 saved / 100 sessions / one week observed) fire
  once each, show at most one at a time, and retire silently on
  established installs. A custom 404 page and a route-keyed error
  boundary keep the app shell alive when a single page crashes.
- **Quality-of-life.** A restart-pending banner that only clears on a
  real daemon restart (plus daemon uptime); a prune-now button
  (`POST /api/prune/run`); config undo (every dashboard config save
  keeps a `.bak`; one click swaps it back); Settings deep-links
  (`?section=`); an empty-state sweep with do-something buttons;
  suggestion cards gained navigate-to-the-fix actions; bare `observer`
  prints a status welcome instead of usage; `observer start`
  auto-opens the dashboard on a TTY (`--no-open` opts out); and
  `observer doctor` ends with `all checks passed ★` when they did.

### Added — live view, global search, budgets, experiments, privacy center, monthly statement

The arc's feature headliners, all local-only like everything else:

- **Live session view** (`/live`, `GET /api/live`): the most recently
  active sessions (≤8) with lifetime proxy-deduped cost/token rollups
  and a live action feed; polls every 5s only while the tab is
  visible.
- **Global search** (`/search`, `GET /api/search`, linkable `?q=`):
  full-text search over captured tool-output excerpts through the
  existing FTS5 index, with snippet highlighting and session context.
  Human browsing through search results does not feed the
  redundancy/freshness signals the observer computes for AI reads.
- **Budget guardrails** (`[intelligence].project_budgets_usd`, a
  root-path → USD map): per-project and global monthly budgets
  surfaced as banners at 80% and 100% (once per month per scope) and
  editable from Settings. Advisory only — nothing is ever blocked —
  and read fresh, so edits apply without a restart.
- **Profile experiments** (`[[experiments]]` + `observer experiment`):
  productized A/B testing for compression profiles. Sessions are
  hash-split into arms — no persisted arm state; reports recompute
  membership — and an active experiment outranks profile assignments
  and overlays for its traffic class. `observer experiment` and
  `GET /api/experiments` share one report engine. This is the same
  mechanism the profile content-refresh runs (A1/A2/A3) used by hand,
  now first-class.
- **Privacy center** (`/privacy`): a capture map of exactly what is
  and isn't recorded (with live config flags), a scrub tester that
  runs the secrets scrubber over pasted text entirely in memory
  (`POST /api/privacy/scrub-test` — nothing stored), and org-push
  transparency including the last enrolment payload, verbatim.
- **Monthly cost statement** (`/report`, `GET /api/report/monthly`):
  a print-friendly monthly statement — totals, spend by model / tool
  / project, savings telemetry (compression bytes and cache-read
  volume, deliberately not a dollar claim), top sessions by cost.
  Print chrome is hidden, so the browser's save-as-PDF produces a
  client-billable artifact.
- **Spend-spike detector**: the advisor's 18th detector warns when
  today's spend runs ≥2× the baseline of your prior *active* days
  (with a $5 floor) and links straight to the live view to see what
  is burning.

### Added — demo mode

Just looking? The dashboard can now seed itself with a synthetic demo
dataset: embedded fixture sessions are parsed through the real
claude-code adapter into a **temporary database**, timestamps rebased
to look fresh. The real `observer.db` is never read or written on any
demo path — the dashboard atomically swaps its data handle to the temp
DB and back. Data surfaces (sessions, cost, suggestions, live view,
search) show demo content under a persistent banner; operational
surfaces (doctor, watcher health, backfill status, connected tools)
deliberately stay real so demo mode never misreports the install's
health. Start from the empty-Overview offer or
`POST /api/demo/start|stop` (idempotent); stopping deletes the temp
DB.

### Added — storage manager + `observer db stats|vacuum|backup`

`observer db stats` breaks the database file down per user-visible
table — index and FTS5 shadow-table bytes folded into the table they
serve, SQLite internals bucketed — so "what is taking 400 MiB" has an
answer. `observer db vacuum` reclaims free pages and reports bytes
freed (honest limitation: it can report SQLITE_BUSY against a busy
daemon; re-run in a quiet moment). `observer db backup <path>` takes
an online snapshot via SQLite's `VACUUM INTO` — safe while the daemon
runs, and it refuses to overwrite an existing file. The same three
operations live in Settings → Storage (through the shared job
registry), alongside a documented restore procedure. Restore is
deliberately manual: stop the daemon, swap the file, start.

### Added — `observer db import` merges another observer.db

`observer db import <path> [--dry-run] [--json]` merges a second
observer database into this node's — built for the stranded-DB case
where a previous install (other OS, other home directory, Windows
side vs WSL side) accumulated history worth consolidating. The source
is first migrated to the current schema (it is opened read-write for
this — work from a copy), then ATTACHed read-only and merged in one
transaction of set-based INSERT…SELECTs: idempotent re-runs (natural
unique keys where the schema has them, conservative NOT-EXISTS
matching where it doesn't), project identities remapped by root path,
cross-row references re-attached, FTS re-indexed after commit.
`--dry-run` executes the same transaction and rolls it back, so its
per-table counts are exact, not estimates. Node-local state (cache
tracking, the session-PID bridge, watcher cursors, file-freshness
state, advisor state) deliberately does not transfer.

### Added — interactive `observer init`

`observer init` with zero flags on a real terminal now runs an
interactive checklist instead of writing everything at once: it
detects the installed AI tools, previews each pending config write
(the same dry-run rendering the dashboard wizard shows), and asks for
consent one write at a time. MCP registration is never pre-selected
and carries the ~1,800-token per-turn overhead note. EOF or ctrl-C
aborts cleanly, having written only what was already approved. Any
flag or redirected stdin/stdout keeps the existing batch behaviour,
so scripts are unaffected.

### Fixed — batch `observer init` writes the Claude Code proxy route it documented

v1.8.2 added the Claude Code proxy-route write (`env.ANTHROPIC_BASE_URL`
in `~/.claude/settings.json`) to the org-enroll wiring path and the
docs described `observer init` as writing it too — but the batch init
loop never gained the write. Batch `observer init` now writes the
claude-code route by default, exactly as documented; `--skip-proxy-route`
opts out and prints the manual hint instead.

### Fixed — `observer claude` no longer re-exports an expired OAuth token

The `observer claude` wrapper re-exports the stored Pro/Max OAuth
access token as `ANTHROPIC_AUTH_TOKEN` so routed traffic stays on the
OAuth path. When the stored token had *expired*, re-exporting it was
actively harmful: a stale env token suppresses Claude Code's own
refresh flow, and every request 401s until the user re-authenticates.
The wrapper now detects the expired case and launches with
`ANTHROPIC_BASE_URL` only — Claude Code refreshes its token itself.

### Fixed — Settings compression sub-panels showed zeros and dropped saves

The six per-group compression sub-panels in Settings resolved their
fields against the wrong config objects after an earlier field-path
refactor: they displayed zero values regardless of the real config,
and saves through them silently no-op'd. Field paths are now
prefix-inclusive, the convention is documented at the registry, and a
test pins each group panel to the object it edits.

### Fixed — `observer profile set` no longer materializes zeros

Writing one key through `observer profile set` round-tripped the whole
profile struct, so every key the operator had never set was written
back explicitly as its zero value — silently turning "unset, inherit"
into "explicitly zero" for the whole file. Profile writes are now
presence-preserving: only the keys already present in the file, plus
the one being set, are written.

### Fixed — Windows-bridged hooks no longer poison tool/cwd resolution

On a Windows-AI-tool + WSL-daemon install, the `wsl.exe`-bridged hook
processes registered init-class PIDs into the session-PID bridge. Any
hookless tool connecting to the proxy afterwards (Codex) then resolved
to the wrong tool identity and the poisoned entry's cwd — e.g.
`tool=claude-code` with a Windows path — corrupting per-tool profile
assignment and per-project overlay resolution. The hook-side process
walk now stops at init-class ancestors, the resolver ignores PID ≤ 2,
and hookless tools gained a `/proc/<pid>/cwd` fallback that restores
per-project config resolution for tools that never run hooks.

### Fixed — concurrent first-open no longer fails with SQLITE_BUSY

Two observer processes opening a freshly created database at the same
moment raced on the initial WAL-mode conversion, and the loser failed
with SQLITE_BUSY. The conversion now retries briefly instead of
failing the open.

### Changed — claude-code profile gains the tools-defs trim (A2-gated)

The `claude-code` compression profile's `compress_types` grows from
`["json","logs","code"]` to `["json","logs","code","tools"]`: tool
**definitions** in the request envelope are trimmed (descriptions of
3+ paragraphs keep the first 2; JSON-Schema `examples` stripped —
names, types, required-ness, enums never touched). A/B-gated per the
profile content-refresh plan: run A2 (2026-06-11, n=8/arm salted
refactors, Opus 4.8, Claude Code 2.1.167) measured **−12.5% total
cost** on valid samples vs the pre-A2 set with **zero**
`tools_changed` cache events across 106 candidate turns — the trim is
byte-stable across turns, so the historical V7-21 re-marshal concern
no longer applies on current binaries. Per-turn cache-read dropped
~32% (the tools block is part of Anthropic's cached prefix). The
master-config default `compress_types` is unchanged; opt out by
pinning `["json","logs","code"]` in a custom profile. Full record:
`docs/plans/profile-content-refresh-ab-plan-2026-06-10.md` §6.

### Changed — compression recipes are now per-provider profiles (Track R)

Recipes are promoted to first-class **compression profiles**, resolved
per traffic class at the proxy boundary instead of overlaying the whole
daemon's config. One daemon serving Claude Code **and** Codex now
applies each tool's tuned parameters simultaneously — Anthropic-path
traffic gets `claude-code`, OpenAI-path gets `codex-safe` out of the
box (editable via the new `[profiles]` config section: `default` +
`by_provider`). Sessions stick to the pipeline they first resolved, so
mid-session config edits never disturb in-flight compression state, and
profile/assignment edits apply to **new sessions without a daemon
restart** (any dashboard config save hot-reloads the router). The
master `[compression.conversation] enabled` switch is unchanged and
profiles never auto-enable compression.

**Deprecated:** `observer start --recipe <name>` no longer overlays the
config load chain; it maps onto `[profiles].default` for the run (all
proxy traffic) and warns. Two side effects of the removal: a dashboard
config save can no longer silently bake recipe values into config.toml
(the save-bakes-recipe footgun is gone), and recipe
`[compression.shell]` / `[compression.indexing]` keys no longer apply
to the watcher side — set those in config.toml directly if you relied
on them.

Later phases of the same arc extended the resolution chain and made
profiles fully user-ownable: a **per-tool assignment tier**
(`[profiles.by_tool]`, resolved from the session's tool identity,
outranking `by_provider`); **per-project overlays**
(`<project>/.observer/config.toml` — allow-listed by construction and
deliberately one-directional: a project file can turn compression OFF
for that project's sessions, never on); **custom profiles**
(`observer profile create --from <builtin> | set | delete`, stored
under `~/.observer/profiles/`, with the same CRUD in Settings;
built-in profiles stay immutable); plus `observer profile
list|show|assign` (including `assign tool:<name> <profile>`) and a
general `observer config set <dotted.key> <value> [--project <root>]`
setter that goes through the single config-write owner. Full
precedence per traffic class: experiment arm > `by_tool` >
`by_provider` > `default`, with the project overlay applied off-only
on top of whatever profile resolved.

### Added — model routing layer (P0–P2)

An opt-in, advise-first model-routing layer
(`docs/model-routing-spec.md` §R1–R26; operator doc
`docs/model-routing.md`) that picks the right model per kind of work
from observed evidence: a turn-kind classifier and tier table feed a
table-driven policy engine (six templates + custom rules, 14 lint
checks, cache-priced switch economics so stickiness comes from math,
not timers), wired to live traffic through two channels — **A** writes
each AI tool's own config (`observer routing apply`), **B** rewrites
the `model` field of proxied requests, gated by the `[routing]` master
switch (default OFF; `advise` logs decisions and touches nothing,
`enforce` rewrites same-provider-shape models only). Around the core:
effort downshifts with zero cache loss, reliability fallback chains +
key pools + local upstreams (Ollama / LM Studio / vLLM), an hourly
calibration job with evidence-gated auto-demote, counterfactual
`routing simulate`, a savings/shadow/explain CLI family, a dashboard
**Routing** page, and the §R17.5 MCP pair. Fail-open everywhere: a
routing failure can never break a turn that would have succeeded.
Privacy posture matches Teams (§R23): decision logs
(`router_decisions`) are node-local and privacy-sentinel-pinned; orgs
receive only an aggregate summary behind a node-side
`[org_client.share].routing_summary` opt-in (default false), and the
signed org policy registry has **no remote enforce toggle** — org
bodies' `enabled`/`mode` keys are structurally ignored.

### Added — Suggestions engine (advisor) Phases 1–4

A new **Suggestions** dashboard tab, `observer advise` command, and
`GET /api/suggestions` endpoint surface prescriptive, dollar-quantified
recommendations computed locally from captured activity — every estimate
shows its arithmetic. Four detectors ship: **session balloon** (the
flagship: a session's prompt window crossed ~150K tokens and every later
turn re-paid to carry the history; the nudge shows the fresh-session
counterfactual — worth $25–63 on real sessions during calibration),
**idle re-cache** (turns that paid to rebuild a prompt cache that expired
during a >5-minute break), **long-context crossings**, and
**trivial-model over-provisioning** (small sessions run entirely on Opus,
migrated from the Analysis tab's routing rule). Thresholds were calibrated
against a real two-month corpus (`docs/plans/advisor-calibration-2026-06-10.md`);
savings apply a 30% haircut for context the user genuinely needed.
Default-on (`[advisor]` config section), read-layer only, zero LLM cost.
Phases 2–4 (same day): thirteen more detectors (cache hit-rate /
write-waste / prefix-thrash on cachetrack verdicts, read-heavy +
effort over-provisioning, fast-tier premium, unrecovered-failure time
sinks with a noise gate, MCP schema tax, compression-off, not-proxied,
wasteful stale reads on the migration-038 split, quality regression,
web-search spend); dismiss/snooze with a 7-day cooldown (node-local
`advisor_state`, privacy-invariant-pinned); the in-session prevention
loop (a 14th MCP tool `get_suggestions`, freshness-tool advisories, and
an opt-in session-start digest injection — default off); and
`observer advise --digest`. Operator reference: `docs/advisor.md`.

### Added — cache tracking P0 closed

Anthropic prompt-cache observation, attribution, and forecasting now
ships P0 complete. The cachetrack engine (`internal/cachetrack/`)
observes every Anthropic Messages API request through the proxy,
enumerates blocks (tools → system → messages), deep-canonicalizes
each block (sorts nested map keys at every depth, strips
`cache_control` recursively, preserves numeric form), hashes a
rolling SHA-256 chain, and attributes per-turn outcomes against a
12-row decision table covering 9 kinds × 14 causes. Three NODE-LOCAL
tables back it (migrations 036 + 037): `cache_segments` (chain
points of interest), `cache_entries` (modelled live provider cache,
keyed by `UNIQUE(model, cache_scope, prefix_hash)`), and
`cache_events` (per-turn verdict with cause + diverged_seq +
predicted_kind + message_id). Default-on per spec §11; the privacy
sentinel at `tests/invariant/privacy_test.go::TestSelectUnpushedSinceExcludesCacheTables`
fails loudly if any of the three tables ever appear in the org-push
wire path.

Three operator-facing surfaces:

- `observer cache-health` reports the §10 mispredict-rate gate (≤
  5% over ≥ 200 graded events at P0) plus two secondary checks: a
  read:write consistency check that flags `*_rewrite` /
  `compaction_reset` events with `tokens_read > 3.0× tokens_written`
  (mechanically impossible shape for a real invalidation), and a
  cause-concentration WARN when a single non-`suffix_growth` cause
  exceeds 80% of graded events. Neither secondary check changes the
  rate gate's pass/fail; both surface the class of error the rate
  is blind to.
- `observer backfill --cache-rescan` re-walks claude-code transcripts
  through the Tier-2 cache engine to retrofit cache attribution onto
  historical sessions — useful after enabling `[cachetrack].enabled`
  with prior traffic, or after a cachetrack fix lands. Idempotent
  end-to-end via the cross-tier `CacheEventExistsForMessage` dedup
  gate (proxy-already-observed turns short-circuit on Tier-2). Picked
  up by `--all`.
- `[cachetrack].calibrate_log_path` enables a per-block JSON-Lines
  sidecar (one entry per block fed to `ObserveTurn`, with sha_raw +
  sha_canon + bounded canonical-bytes prefix for tools/system
  levels). Auto-stops after 200 blocks. Diagnostic tool for
  localizing chain-hash drift; the actual fix path for the P0 soak.

P0 closed at the 2026-06-09 soak: 147 graded events at 2.7%
mispredict rate (operator-accepted; the 200-event minimum deferred
as arbitrary), 0 `system_changed`, 0 inconsistent rewrites, no
dominant cause.

The P0 arc surfaced and fixed three latent defects worth highlighting:

- **Shallow canonicalize.** The pre-fix `CanonicalizeAnthropic` sorted
  only top-level keys and preserved nested objects byte-verbatim from
  the source RawMessage. Any nested key-order variation upstream
  would have produced a per-turn chain-hash drift. Fix is a deep
  `interface{}` round-trip with `json.UseNumber` — Go's encoding/json
  sorts `map[string]any` keys at every depth on `Marshal`. Equivalence
  test pinned: byte-different logically-equal tool defs canonicalize
  identically.
- **Per-request telemetry block.** Claude Code 2.1.169+ prepends a
  `{"type":"text","text":"x-anthropic-billing-header: ... cch=<NONCE>;"}`
  block at `system[0]`. The `cch=<NONCE>` changes every request, but
  Anthropic excludes it from cache identity (provider hits cache every
  warm turn despite the nonce churn). We were hashing what the
  provider ignored — the cumulative chain absorbed the nonce into
  every downstream segment, false-firing `system_changed →
  invalidation_rewrite` on every healthy warm turn (198/201 events
  on the f81cec3f soak, 13/15 on 39326aa9). Fix excludes the block at
  the `EnumerateAnthropicBlocks` system walk via prefix-match on
  `x-anthropic-billing-header:` (cc_version / cc_entrypoint may
  evolve, cch= is unbounded). Same shape as the canonicalize-side
  `cache_control` strip: provider-excluded telemetry, identity-
  neutral on both sides.
- **MispredictRate counted nothing.** `bucketOf` placed `KindMispredict`
  itself into `bucketSkipped` (correct for ReconcilePrediction's
  predicted-vs-observed bucket diff which demotes the matched entry).
  `MispredictRate` shared the same predicate so KindMispredict events
  were `continue`'d at the top of the loop AND the
  `if k == KindMispredict` numerator increment became dead code. The
  5% rate gate had silently graded every soak as 0.0000 since C9.
  Fix introduces a separate `isRateSkipped` predicate for the rate
  vocabulary (BelowMin / Reanchor / CompactionReset only;
  KindMispredict IS counted).

### Added — zero-usage rate carve-out

Follow-up post-P0: the 4 opus-4-8 `unknown` mispredicts on the
2026-06-09 soak were diagnosed as **zero-usage envelopes** (the
provider returned `input_tokens=0, output_tokens=0,
cache_read=NULL, cache_creation=NULL, http_status=NULL` — short
cancelled or partial streams). The engine emits `KindMispredict +
CauseUnknown` row-12 fallthrough because no rule matched AND no
usage signal arrived, but the turn is observationally vacant: no
token data to grade the engine's prediction against. Counting
these as mispredicts distorts the rate the same way the C9 rate
bug did.

Fix: `cachetrack.MispredictRateGraded` (new) skips KindMispredict
events whose `tokens_read=0 AND tokens_written=0` from BOTH the
numerator and denominator. `MispredictRate` (simple form) stays
for callers without per-event token data. `observer cache-health`
switched to the graded variant; rate on the same soak goes from
4/147=2.7% → 0/143=0.0% (more honest — the 4 events were
ungrade-able). The MispredictEvents diagnostic list still
includes them, now annotated `[zero-usage, excluded from rate]`
so operators can see the row-12 fallthroughs happened without
them distorting the gate.

Known limitation documented in
[`docs/cache-tracking.md#known-limitations`](docs/cache-tracking.md):
a small MCP `tools_changed` read:write WARN residual (~2 events
per ~150 opus-4-8 turns). Likely a real `tools_changed` (MCP
server connect/disconnect) where the prior cached prefix still
warm-reads the older tools-array slice; cause IS correct but the
read:write guard's 3.0× threshold may want a per-cause carve-out.
Revisit when MCP toggles dominate.

Operator reference at [`docs/cache-tracking.md`](docs/cache-tracking.md).
Implementation spec at
[`docs/plans/cache-tracking-implementation-spec-2026-06-08.md`](docs/plans/cache-tracking-implementation-spec-2026-06-08.md).

Files: `internal/cachetrack/`, `internal/store/cachetrack.go`,
`internal/store/store.go` (`SetCacheEngine` + Tier-2 wiring in
`Ingest`), `internal/proxy/proxy.go` (`CacheSink` interface),
`internal/adapter/claudecode/adapter.go::tier2Accumulator`,
`cmd/observer/cache_health.go`, `cmd/observer/backfill.go`
(`--cache-rescan` flag), `cmd/observer/proxy.go::buildProxy` (single
engine instance fed to proxy.New AND store.SetCacheEngine),
`internal/db/migrations/036_cache_tracking.sql`,
`internal/db/migrations/037_cache_events_message_id.sql`,
`internal/config/config.go::CacheTrackConfig`,
`tests/invariant/privacy_test.go::TestSelectUnpushedSinceExcludesCacheTables`.

### Added — server-side global Sessions sort

Clicking a Sessions-table column header now sorts the **whole filtered
set** on the server and then paginates — not just the loaded page.
Previously the table sorted client-side over one ≤50-row page, so "sort by
cost descending" surfaced the most expensive of the *visible* rows, never
the most expensive session overall (anyone with more than one page of
sessions was affected). `/api/sessions` gains `sort_by` / `sort_dir`
(allow-listed columns — SQL `ORDER BY` for cheap columns, a global
in-memory sort for the token/cost buckets) and returns `page_cost_usd` /
`page_ai_cost_usd` / `page_tool_cost_usd` so the footer reconciles with the
visible rows. The frontend `DataTable` gained an optional controlled
(`manualSorting`) mode. Per the `srinivasdrona` fork audit §7.C.

### Added — unpriced-token observability in the cost summary

`cost.Summary` now reports `unpriced_tokens` + `unpriced_turn_count`
alongside the existing `unknown_model_count`. These volume-weight the rows
that resolved to a pricing MISS (empty / unknown model, billed $0) so a
whole-adapter attribution regression is loud rather than hidden behind a
small distinct-model count. Per the fork audit §11.B.

### Fixed — cost engine rescues router- / provider-prefixed model strings

`LookupWithSource` now applies a last-resort normalization *after* the
exact / `:free` / date-strip / family lookups all miss: it strips router
prefixes (`capi:`, `sweagent-capi:`) and leading provider path segments
(`openrouter/anthropic/foo` → `foo`) and retries exact + family once. This
rescues model strings that previously hit a pricing MISS → $0 for *any*
adapter, while curated host-rate provider-qualified keys
(`deepseek/deepseek-v4-flash`) still win first. `auto` and the empty string
correctly remain a MISS (neither names a real model — their cure is
adapter-side). Per the fork audit §11.A.

### Fixed — GitHub Copilot CLI cost attribution (empty / `capi:`-prefixed model)

Copilot CLI process logs that lack the `[INFO] Using default model: …`
line emitted Tier-1 token rows with an **empty model → pricing MISS → $0
cost** (token counts were captured correctly; only the cost silently
dropped to zero). The adapter now recovers the model from the response
body's `"model"` field and, as a last resort, from the sibling
`session-state/<id>/events.jsonl` `selectedModel`, stripping Copilot's
`capi:` / `sweagent-capi:` routing prefixes (the dashed `claude-opus-4-7`
form is preserved — our pricing table keys on it). Historical rows already
persisted with an empty / `capi:` model re-attribute via
`observer backfill --copilot-cli-rescan`. Per the fork audit §7.A.

### Fixed — copilot token output double-counted

The copilot family (`copilot`, `copilot-cli`) emits **two `token_usage`
rows per turn** — a full-usage row plus a separate output-only row from a
second source file — which the adapter intended to merge on
`(session_id, message_id)`, but the store keys on
`(source_file, source_event_id)`, so they never merged and the turn's
output double-counted (a Copilot CLI session showed 304 output tokens for a
152-token turn). A scoped rollup-layer dedup now drops the output-only
"shadow" row when a full-usage sibling in the same session carries the same
output, at both surfaces that sum tokens (the cost engine + the
session-detail / messages CTEs). Gated to the copilot tools, so no other
adapter is affected.

### Fixed — GitHub Copilot CLI "Request-ID null" loses the turn's message id

Modern Copilot CLI (1.0.60+) process logs print `[DEBUG] response
(Request-ID null)` — the hex:colon Request-ID the older format carried
inline is gone — so the Tier-1 token row landed with the literal string
`"null"` as its `message_id` (visible in the messages timeline, and it broke
the `(session_id, message_id)` link to the Tier-3 events.jsonl row). The
adapter now recovers the real Request-ID from the sibling
`session-state/<id>/events.jsonl` `assistant.message.requestId`, matching
each `"null"` row to its event by output-token count + nearest timestamp.
This also restores the store-layer shadow merge **at ingest**: with the
Tier-1 and Tier-3 rows now sharing a `message_id`, the existing
`(session_id, message_id)` sweep drops the output-only shadow directly (the
rollup-layer dedup above remains a belt-and-suspenders fallback, and still
covers the VS Code `copilot` adapter). The `token_usage` UPSERT additionally
heals an existing `null`/empty `message_id` to a recovered value, so
`observer backfill --copilot-cli-rescan` fixes sessions that were already
ingested with the placeholder.

### Fixed — GitHub Copilot CLI cache-write tokens billed as input

Modern Copilot CLI (1.0.60+ serving Anthropic-via-Bedrock turns) reports
`cache_creation_tokens` inside `prompt_tokens_details`, and its
`prompt_tokens` is the GROSS prompt = input + `cached_tokens` (cache read) +
`cache_creation_tokens` (cache write). The Tier-1 parser netted out only the
cache-read portion, so the cache-write tokens were billed as net input (a
live turn: 16257 of a 16267-token prompt). The parser now captures
`cache_creation_tokens` and nets it out of input, surfacing it in its own
column so the cost engine bills it at the cache-write rate. The
`token_usage` UPSERT additionally lets `observer backfill
--copilot-cli-rescan` correct a previously-ingested row's input *down* when
it newly discovers cache-write tokens (scoped to the copilot tools — every
other adapter keeps the monotonic `MAX`, so a partial re-parse can't lower a
complete count).

### Fixed — Sessions tab undercounted long-running sessions vs the Cost tab

`/api/sessions` windowed sessions on `started_at` only, while `/api/cost`
and `/api/models` window by turn/token timestamp. A session that started
before the window but had recent turns therefore appeared in the Cost tab
but not the Sessions tab. The window predicate now also matches a session
with any in-window activity (action / `api_turn` / `token_usage`), so the
two tabs reconcile; the pagination total stays consistent. Per the fork
audit §6.B.

### Fixed — Sessions calendar per-tool cost

`/api/sessions/calendar?tool=X` filtered the per-day session *count* by
tool but rendered **all-tools** cost beside it, overstating per-tool spend
on the heatmap. The per-day cost rollup now honors the `tool` filter. Per
the fork audit §7.E.

### Fixed — empty marker-only sessions no longer surface on the dashboard

A Windows Claude Code probe session (CLAUDE.md loads into context, a
`settings.json` is touched, or a session is opened then closed without
work) fires a single hook event — `instructions_loaded` or
`config_change` — which routes through the `wsl.exe` bridge into the
WSL daemon DB. `store.Ingest`'s per-event `UpsertSession` then
bootstraps a contentless session whose only row is that one marker:
no tool calls, no `token_usage`, no `api_turns`. These surfaced on the
Sessions tab as ghost rows (e.g. `total_actions` 0/1, never ending).
Same class as the v1.8.2 `session_end` phantom fix, different trigger.

Unlike `session_end` (terminal — the real session always exists by the
time it fires), `instructions_loaded` / `config_change` fire **at
session start, before the watcher has parsed the transcript**, and they
legitimately attach to real sessions too — so they can't be dropped at
ingestion without losing data. The fix is a read-layer filter: a session
surfaces only when it has at least one substantive action OR any
`token_usage` OR any `api_turns` row. Applied as one shared predicate to
`/api/sessions` (list + pagination total + scored-count), the overview
session stat, and the calendar per-day counts so all three agree.
Non-destructive — `instructions_loaded` / `config_change` stay attached
to real sessions, only the contentless probes are hidden. Existing probe
rows already in a DB are cleaned out-of-band; new ones never appear.

Files: `internal/intelligence/dashboard/dashboard.go`
(`emptySessionMarkers` + `nonEmptySessionPredicate{S,Sessions}`
compile-time consts, wired into the three count paths),
`internal/intelligence/dashboard/dashboard_test.go`
(`TestAPISessions_HidesMarkerOnlySessions`).
...
Captures session data from both [Kilo Code](https://kilo.ai/) products:
...
**Token capture.** Per-assistant-message bundle on `message.data.tokens
= {total, input, output, reasoning, cache: {read, write}}` — mirrors
...
Agent](https://github.com/NousResearch/hermes-agent) — the open-source
...
Captures session data from [Cline CLI](https://docs.cline.bot/) — the
...
`phase-4-arcade-split` → `phase-4b-state-difficulty` → `...` →
`phase-4j-konami`.
...
palette (`CHAR_PAL`), 3×3 chest emblem (`CHAR_EMBLEM` — `S` /
...
- `npm/observer/README.md` and `pypi/observer/README.md` shared
  ...
  `make sync-distribution-readmes` regenerates; `make
  verify-distribution-readmes` is a CI drift gate that builds into
...
- `npm-release.yml` gains a top-level `concurrency:` block
  (`group: npm-release-${{ github.ref }}`, `cancel-in-progress:
  false`). Defends against re-pushed tags racing on artifact
...
- New `vscode-preflight` job queries the Marketplace via `vsce
  show superbased.superbased-observer --json` BEFORE the
  ...
  errors. `vscode-package`'s `needs:` is now `[build,
  vscode-preflight]`. With v1.8.1's G8 stamp fix, the v1.8.0
...
- **deploy/observer-org IdP boots again (N1).** The v1.8.0
  ...
  replace with a compose-level entrypoint that `sed`s the pin
...
- **IdP healthcheck no longer reads PHP fatals as Healthy (N3).**
  ...
  was malformed. New check pipes the response through `grep -q
  EntityDescriptor`, so an unparseable metadata body fails
...
- **SAML resolver detaches from r.Context() (5b).** Issue 5b's
  ...
  the DB query, surfaced as `api.store.memberByEmail: context
  canceled`, and crewjam translated the failure to 403. Same
...
- **Dashboard renders data again (5b dashboard half).** Even
  ...
  `[org]` section at all. Fix: dev config now ships `admin_emails
  = ["user1@example.com", "admin@example.com"]` so both the
  kristophjunge default user and the quickstart-provisioned
  admin can see whole-org rollups.
- **`observer-org quickstart` survives cold `docker compose
  build` (N2).** The whole flow ran under a 120s budget; a cold
...
- **Server-side `actions.source_file` ambiguity documented
  (N5).** Migration files are immutable once shipped, so the
...
- **N4 caveat — proxy-down warning + symmetric unenroll cleanup.**
  ...
  - Enroll now does a 200ms TCP-dial probe of the proxy port
    after the settings.json write and emits a loud `WARNING:
    observer proxy is not running` block with the remediation
    command.
...
- **D1 — Projects rollup now works under default privacy posture.**
  ...
  - `internal/orgserver/rollup/cost.go::spendCTE` now reads
    ...
    with `LEFT JOIN sessions s ON s.id = t.session_id AND
    s.user_id = t.user_id` so proxy-fed `api_turns` whose own
...
- **Contact-modal cleanup** (`22ef9b5`). Replaced the three founder
  ...
  `contact@superbased.app` row + inline X/LinkedIn/YouTube/GitHub
  socials. Personal founder emails removed (kept private).
  ENTERPRISE plan CTA mailto also swapped to `contact@superbased.app`.
...
- **Live verification corner cases captured in memory:**
  ...
  grep for `contact@superbased.app` returns 0 even when correct —
...
- v1.8.0 ships **sha256-hex hashes** (`target_hash`, `source_file_hash`,
  ...
  operator** opts in via `[org_client.share].full_content = true` in
...
- The push cursor seeds at the current high-water id at `observer
  enroll` time (already true since v1.7.x; M2.1 of the remediation
...
- The OS-keychain-unavailable WARN now fires once per host downgrade
  ...
  collapsed across files so the initial-scan burst on `observer
  start` is bounded.
...
- `docs/plans/teams-test-findings-remediation-plan-2026-06-03.md` is the
...
- `vscode/` — full TypeScript extension under `superbased.superbased-observer`
  ...
  - **Three-mode daemon lifecycle** (`observer.daemon.mode = detect |
    managed | auto`) with lockfile safety so the extension never
  ...
  - **Native sidebar** with four `TreeView`s: Today (60 s), Sessions
...
- **`/api/file/state?path=<abs>`** — new dashboard endpoint
  ...
  `{last_read_at, last_read_by, edit_count_24h, stale_rereads_24h,
  tools_touched[]}` for one file. Four single-row queries against the
...
- `pypi/observer/pyproject.toml` — explicitly excludes
  `src/observer/_bin` from the `[tool.hatch.build.targets.wheel]
  packages` glob and pulls the binary back in via
...
- `pypi/observer/` package source layout with hatchling-based
  ...
  (`observer/__main__.py`) that `os.execv`s the bundled binary,
  and a per-platform wheel build flow mirroring the npm matrix.
- `publish-to-pypi` job in `.github/workflows/npm-release.yml`:
  builds the 5 wheels and uploads via `PYPI_TOKEN_OBSERVER` +
  `pypa/gh-action-pypi-publish`. Runs in parallel with the npm
  `publish` job; gated to the private repo same as the rest.
- `pypi/observer/README.md` long-description rendered on PyPI
  (mirrors npm README with pip-flavored install + troubleshooting
  sections).
- Root `README.md` install section gains a pip / uv / pipx
  sub-section alongside the existing npm sub-section.
- npm `README.md` adds a one-line cross-reference to the PyPI
  package so users see both options.
- `docs/plans/pypi-package-plan-2026-06-02.md` captures the full design
...
- No schema migrations, no API changes, no behavior changes for
  existing users.
- `pip install` and `npm install -g` produce byte-identical
  `observer` binaries — the wheel + tarball pull from the same
  CI artifact.
...
- **Migration 032** — `token_usage` adds `turn_id TEXT` column
...
binary with the `claude-code` recipe `compress_types =
["json","logs","code"]`, `mode = cache_aware`) measured a **+60% cost
...
Plan + BC analysis:
`docs/plans/v1.7.22-claude-code-passthrough-plan-2026-06-01.md`.
...
- **New: `docs/skills/observer-cost-tuning/`** — a Claude Code skill
  ...
  presents two operator tiers (**hooks-only** = `observer init
  --claude-code --skip-mcp` → 0% overhead; **full MCP** = default
...
remained** vs the no-proxy baseline. Cache-hit ratios across the n=8
...
prior-turn bytes carried forward** — net-new input per turn is ~14%
larger than no-proxy. The hit is universal across providers, not
codex-specific: Claude Code on the default `compress_types =
["json","logs","code"]` recipe paid the same overhead (just hidden
...
preserve the trim explicitly opt in via `compress_types = [...,
"tools"]`. Plan + BC analysis:
`docs/plans/v1.7.21-tools-defs-gate-plan-2026-06-01.md`.
...
- **`compressToolDefinitions` gated behind `compress_types` at all
  three pipeline sites** (`internal/compression/conversation/
  pipeline.go:358` Anthropic, `:447` OpenAI Chat, `:509` OpenAI
...
- **`TestPipeline_EmptyCompressTypes_SkipsAllPerTypeCompression`
  extended to assert no `tools` mechanism event** — the V7-19 +
...
- **Cold-cache regression risk** — `compressToolDefinitions`'s own
  doc comment says: "*on warm turns it produces zero net savings on
  the wire (the tools field is cached either way)*". The savings
...
- **The default `CompressTypes` list in `internal/config/config.go:
  703` is unchanged.** Tools is opt-in everywhere.
...
`if priorMonthSameDayEnd.After(monthStart) { priorMonthSameDayEnd =
monthStart }` at `internal/intelligence/dashboard/analysis.go:71`.
...
Headline (v1.7.19): **Critical** — `buildAllow([])` nil-trap silently
...
- **The always-on transformers (tools-defs trim + C16 read-cache
  substitution) are NOT under `buildAllow`'s gate** and continue to
...
- **V7-19** — `buildAllow([])` nil-trap inverts codex-variant recipe.
...
Bug 2** populates `compression_events.body_hash` on the `tools`
...
- **V7-18 Bug 2 BodyHash on tools-mechanism Events**: the three
  tool-definition compressor sites — `internal/compression/
  conversation/anthropic.go::compressToolDefinitions`,
...
- **V3-4 hook-event log**: every observer hook invocation appends one
  JSONL row to `~/.observer/hook-events.jsonl` with `{ts, event,
  bytes, session_id?, tool?, action}`. Per-process mutex + O_APPEND
...
- **Live regex parser (item 2 + 4 foundation)**: new
  `internal/codegraph/livesym` package with `Parse(path) → (syms,
  langUnsupported, err)`. Supports Go (function / method / type /
...
- **`include_nearby` opt-in (item 1 — naming overlap with V7-17 item
  1 unintentional)**: get_symbols requests can set `include_nearby:
  N` (>0) to get `nearby_symbols: [...]` per match — up to N before
...
- `compression_events.body_hash` column (migration 031) — sha256-hex
  ...
  Partial index `idx_compression_events_turn_body_hash WHERE
  body_hash IS NOT NULL` keeps the index small. (V7-9.)
...
- Audit semantics (V7-14): one `mcp_audit` row per resolved sha.
  ...
  sha as `stashed://<sha>` so `WHERE path_requested LIKE
  'stashed://%'` queries sweep retrieve_stashed activity uniformly.
...
- New `mcp__observer__get_file(project_root, path, start_line?, end_line?)`
  ...
  true (default). Returns `{ok, path, project_relative_path, lines,
  body, size_bytes, truncated}`. Pairs with the v1.7.7 marker
...
- New `mcp_audit` SQLite table (migration 030) + writer package
  ...
  is the agent reading?" — queryable today via `sqlite3 ~/.observer/
  observer.db 'SELECT ...'`; managed CLI deferred to v1.8.x.
...
models reading `… [231 lines elided: 3 ERRORs,
12 WARNs]` distrust the marker and re-derive via shell tools — the
...
`… [231 lines elided from src/components/Editor.tsx: 3 ERRORs,
12 WARNs; preserved 5 anomalies; file contains fn handleClick,
class Editor, fn useEffect (codegraph)]`. Both enrichment layers are
...
- `compression_cost_saved_usd_est` USD column now weights by the
  ...
  codex / OpenAI sessions**: this column drops ~10× to match
...
- `docs/plans/v1.7.6-codex-friendly-compression-plan-2026-05-30.md` plan
...
Headline: three follow-on fixes for `docs/observer-platform-
issues-v6.md` findings surfaced during v1.7.4 verification against
...
1. **Pre-warm at `observer start`**: new `Proxy.Prewarm` method
   fires HEAD requests against `[proxy].prewarm_targets` (defaults:
   `https://chatgpt.com/`, `https://api.openai.com/`) via the
   proxy's upstream `http.Client`. This populates the
   `http.Transport` connection pool with warm TLS sessions, so the
   first real codex request reuses an established connection.
   Non-blocking goroutine from `ListenAndServe`; per-target timeout
...
2. **`flusher.Flush()` after `WriteHeader(resp.StatusCode)`** on
   the streaming path only (gated by `isStream`). Defense-in-depth:
   pushes the `HTTP/1.1 200 OK` status line + response headers to
   codex's inner pipe within RTT instead of waiting for the first
   body byte. Doesn't fix the upstream TTFB inside `doWithRetry`
   ...
   client return before the synchronous `insertTurnDetached` lands
...
operator-facing guidance ships in `docs/codex-shared-app-server-
gotcha.md` (public carve-out) where two new sections cover V6-2's
...
heuristic). POSIX path stats `${CODEX_HOME:-~/.codex}/app-server-
control.sock` as a microsecond fast path; if absent, returns empty
...
1. **Process-level watchdog** wired into `newHookCmd`:
   `time.AfterFunc(maxRuntime, os.Exit(2))` armed at the start of every
   `observer hook ...` Run; `defer stop()` disarms on a clean return.
   Default 30 s, overridable via `--max-runtime` and
   `OBSERVER_HOOK_MAX_RUNTIME`. Covers ALL hang causes — DB contention,
   stuck syscall, network — not just one hypothesis.
2. **`db.Options.SkipIntegrityCheck`**: hook subprocesses now skip the
   `PRAGMA quick_check` probe that contended with the daemon's WAL holder
   (the contention the existing `feedback_hook_db_open_no_timeout` memory
...
### fix(release): attach SLSA provenance to the public release

v1.7.0's release workflow generated everything except SLSA provenance: the
reusable generator halts on a private repo (it won't record a private repo's
name in the public Rekor transparency log), so its `final` job exited 27 and no
`*.intoto.jsonl` was produced — while the release notes still advertised it.
Two fixes: (1) `private-repository: true` overrides the guard (the builder
identity is already public via the cosign-verify instructions, so Rekor exposes
nothing new); (2) `upload-assets: false` keeps the provenance as a workflow
artifact, and the `public_release` job now stages the `.intoto.jsonl` onto the
**public** GitHub Release next to the binaries it attests (the generator's own
upload would target the private repo's release). A missing attestation now fails
`public_release` hard rather than shipping a release that falsely claims SLSA L3.
No product code changes — v1.7.1 is v1.7.0 with working provenance.
...
- **Linting tightened.** `.golangci.yml` adds `errorlint`, `gosec`, `gocritic`,
  and `gocyclo`; `gofumpt` replaces `gofmt` as the canonical formatter (`make
  fmt` runs `go tool gofumpt`, pinned via the `tool` directive) and
...
- **Kubernetes.** New Helm chart `charts/observer-org/` (singleton Deployment,
  ...
  `helm` job lints + templates it and `helm install`s into a kind cluster.
...
- **`internal/exporter/otel`** — `Exporter{New,Start,Stop}` tailing `api_turns`
...
- **Row tail** — `store.SubscribeAPITurns` drains `api_turns` in id-order pages
...
- **Opt-in prompt content** — `emit_prompt_content = true` (default false)
...
- **Reference dashboards** — `docs/exporters/otel/` Grafana 11 + Datadog
  dashboards and Prometheus recording rules, with a README documenting the
  Collector `spanmetrics` pipeline they assume.
- **Tests** — `testing/synctest` for the row-tail cadence + restart-from-cursor;
...
- **`/api/org/*` endpoints** — the generated dashboard `ServerInterface`
  ...
  enforced per request in the handler** (defence in depth): an org admin (email
...
- **Drill-down audit** — `GET …/developers` writes a `view_team_developers`
...
- **Server ingest** — `POST /api/agent/push` now enforces the per-push
...
- **Compound enrolment token** — `EnrollResponse` now echoes the resolved
  `user_id` so the agent records its own identity without decoding the bearer.
- **Agent CLI** — `observer enroll <org-url> <token>`, `observer unenroll`,
...
- **Daemon** — `observer start` runs the push loop as a goroutine when org mode
...
- **`cmd/observer-org`** — new binary with `serve` / `migrate` /
  ...
  `make build-orgserver` → `bin/observer-org`.
...
- **OpenAPI** — `docs/openapi/orgserver.yaml` extended with the SCIM,
  SAML, dashboard, and admin surfaces; codegen scoped to the `agent` tag
  so only enroll + push generate stubs.
- **Deploy** — `Dockerfile.observer-org` (distroless static) +
...
- **E2E** — `tests/e2e/orgserver` drives the full flow (SCIM-provision →
...
- `WatchPaths` adds `<home>/.cursor/chats` alongside `…/projects` (for
  every crossmount home → covers WSL + Windows /mnt/c).
- `IsSessionFile` matches `.cursor/chats/<ws-hash>/<conv>/store.db`
  (`matchesStoreDBShape`; excludes the -wal/-shm sidecars).
- `ParseSessionFile` dispatches store.db paths to `parseStoreDBFile`,
  which emits the system-prompt + prompt-budget rows; the transcript
  path no longer emits them.
- Because the store.db files were never watched before, the watcher's
  ...
  refreshes as the store.db grows. Decoupled from transcript growth.
- Project root for these rows is resolved from the sibling transcript's
...
- **cline + gemini** (Class B): both adapters called `git.Resolve`
  on a Windows-shaped cwd without translating, walking up from
  observer's own CWD and landing on observer's `.git` in the worst
  case. Now route through `pathnorm.Normalize` first.
- **copilot**: `decodeFileURI` rewritten in terms of
  `pathnorm.NormalizeWithFormat` with the `FormatFileURI` gate.
  Windows-side workspace URIs now canonicalise to `/mnt/d/...` on
  Linux instead of `d:/...` — strict improvement (stat-able / git-
  resolvable from a Linux observer).
- **antigravity** `decodeFileURIToRoot` (used by recovery output)
...
- **Layout classifier** (`adapter.go`) — new `Layout` enum +
  `classifyLayout()` exhaustively returns `LayoutDesktop` /
  `LayoutCLI` / `LayoutUnknown`. CLI is checked before desktop
  because the desktop substring `/.gemini/antigravity/` is a prefix
  of the CLI form `/.gemini/antigravity-cli/`. `matchesSessionShape`
  + `defaultRoots` cover both layouts under every crossmount home.
- **CLI metadata resolver** (`metadata_cli.go`) — CLI installs ship
  no desktop-style `state.vscdb`; metadata is synthesised by
  parsing `~/.gemini/antigravity-cli/log/cli-*.log` for the
  conversation_uuid → project_uuid binding (the CLI emits two
  consecutive `glog` lines: `Conversation using project ID: <uuid>`
  + `Created conversation <uuid>`), then reading the project's
  `name` + `gitFolder.folderUri` from
  `~/.gemini/config/projects/<project-uuid>.json`. Memoised by
  log-dir mtime via the existing `indexCache` map under a `"cli:"`
  key prefix. `lookupIndexEntry` now dispatches by layout —
  desktop hits `state.vscdb` as before, CLI hits the new path.
- **`agy` gRPC discovery** (`process.go`) — `discoverNativeLinux`
  / `discoverNativeMac` / `discoverViaPowerShell` extended to
  recognise `agy` / `agy.exe` / `antigravity` / `antigravity.exe`
  basenames in addition to `language_server_*`. A new
  `requiresCSRF` policy gates the `--csrf_token` cmdline check:
  desktop `language_server` processes always carry it; CLI
  embedded servers may not (the CLI uses a different localhost
  auth). Empty-CSRF rows for CLI processes now pass through to
  the gRPC fan-out instead of being dropped at parse time.
- **`RetrySuggested` adapter signal** (`internal/adapter/adapter.go`
  + `internal/watcher/watcher.go`) — new bool on `ParseResult`.
  The watcher now persists the cursor (MAX-of-old-and-new) when
  it's set, so fresh CLI `.pb` files that fail decrypt + gRPC on
  the first attempt stay visible to `pollCursors` for retry on the
...
- **Backfill + rescan coverage** —
  ...
  `--antigravity-project-root` / `--antigravity-rescan` flags
  inherit; no new flags.
- **Diagnostics** (`internal/diag/doctor.go`) — new
  `antigravity.family` check reports presence + `.pb` counts of
  both layouts plus the configured `network_recovery` setting.
  Emits StatusWarn when CLI `.pb` files exist but
  `network_recovery` is off (the CLI doesn't ship an oscrypt
  secret equivalent to the desktop install, so local decrypt
  cannot succeed for CLI files — gRPC fallback is the only
  practical recovery). A startup nudge in `cmd/observer/main.go`
  emits the same warning at daemon-start time so the operator sees
  the gap before opening the dashboard.
- **UI auto-refetch** (`web/src/lib/useApi.ts`) — `useApi()` gains
  optional `refreshMs` + `refreshWhenHidden` opts. Pages now poll
...
on every CLI parse and synthesises:
  ...
  - `user_prompt` events for `source=USER_EXPLICIT/type=USER_INPUT`
    entries (with the `<USER_REQUEST>...</USER_REQUEST>` wrapper
    stripped via `extractUserRequestText`)
  - `task_complete` events for `source=MODEL/type=PLANNER_RESPONSE`
    entries with non-empty `content` (assistant text replies)
...
catches user-side messages only, no assistant content. Three exit points were wired:
  ...
  - **decrypt-success path**: tops up emitted events with any
    ...
    the desktop layout — `cliRootsFor` returns empty there).
  - **gRPC-success path**: tops up the bridge's markdown +
    structured merge with history entries the bridge didn't expose.
    This is the primary win: every CLI session that bridges
    *partially* gets the user-typed turns filled in.
  - **decrypt + gRPC double-failure path**: if history.jsonl has
    any entries for the conversation, return them as the entire
    `ParseResult` instead of marking the file unrecoverable. New
    `historyOnlyResult` helper. Brand-new sessions where the
...
- `desktopRootFor` walker (`metadata_cli.go`) — mirrors the existing
  `cliRootsFor` for the desktop layout. Returns the
  `<gemini-root>/antigravity` parent of both the encrypted
  `conversations/` and plaintext `brain/` subtrees.
- `transcriptPathFor` resolver (`metadata_cli.go`) — single entry
  point that picks `transcript.jsonl` vs `overview.txt` by
  `classifyLayout`. Callers no longer repeat the conditional.
- `augmentResultFromHistory` + `historyOnlyResult` (`history_cli.go`)
  — both stopped gating on `LayoutCLI`. The `decrypt → augment` /
  `decrypt+gRPC fail → escape hatch` paths now surface desktop
  overview.txt content. `history.jsonl` fallback remains CLI-only
  (desktop has no analogue).
- Caller gates at `adapter.go:431` + `:507` (`if classifyLayout(path)
  == LayoutCLI`) removed so desktop sessions reach the augmentation
  path.
...
failure (`copy bridge to /mnt/c/.../antigravity-bridge.exe: open ...:
permission denied` — Windows AV scan or another bridge invocation
...
Permanent decrypt failures (`no cipher mode produced a validating
plaintext`) still mark unrecoverable as before — that's a per-file
...
adds `TestDesktopRootFor` (4 subtests), `TestTranscriptPathFor` (4
...
sqlite3 ~/.observer/observer.db \
  "DELETE FROM adapter_unrecoverable_files WHERE adapter='antigravity' \
   AND reason LIKE '%permission denied%';"
```

Going forward, transient bridge errors don't write markers in the
...
in-use-by-running-powershell lock on the `/mnt/c/.../observer/
antigravity-bridge.exe` cache copy that surfaced on c15725c6, but
...
Fix: new `cliLogFreshnessUnix(logDir)` helper returns `max(dir
mtime, latest cli-*.log file mtime)`. Captures both file-create AND
...
1. **Cold bridge timeout too tight**: the 30s timeout killed the
   ...
   no-progress loop.
2. **Cache file lock**: the running observer constantly invokes the
   cached `/mnt/c/.../observer/antigravity-bridge.exe` via
   ...
   new `make build` would change the source size, `copyIfNewer`
   would fail with permission-denied, the graceful-fallback would
   keep using the OLD cached binary, and new bridge code (the
   `--endpoint`/`--csrf` flags this session shipped, plus stderr
   `bridge-endpoint=` hints for cache population) NEVER reached
   production — defeating the conv-endpoint cache entirely.

Fix:
- `bridgeColdCallTimeout = 90 * time.Second` constant for unpinned
  calls; pinned (cache-hit) calls keep the original tighter timeout.
- `windowsCacheDestinationFor(srcSize)` returns
...
- Before: bridge `convert` killed at 30s; transcript fallback fired
  → 2 ToolEvents / 0 TokenEvents / project `[antigravity]`.
- After: bridge `convert` succeeds in ~30s, bridge `structured`
...
Storage: ~150 KB per conversation in steady state. An `observer
doctor` prune step is queued separately.
...
language_server PID + listening ports, plus every `Created
conversation <uuid>` line. Result is a `conv UUID → (PID, HTTPSPort,
HTTPPort)` map. Both `callBridgeConvertCached` +
...
Design doc: `docs/plans/antigravity-token-coverage-design-2026-05-24.md`
...
- New column persists the full untruncated tool_result body per
  action, mirroring the existing `raw_tool_input` length-merge
  ON CONFLICT semantics so adapter re-scans keep the richer of the
  in-flight vs final body without ever regressing.
- `UpdateActionOutcome` also writes the column on post-hook merge
...
- New `internal/contentcap` package — `Cap(s, max)` returns the body
  with a trailing `…(content truncated at N bytes)…` marker if it
  exceeded `max`. `DefaultMaxBytes = 1 << 20` (1 MiB).
- `internal/scrub.MaxRawInputBytes` bumped from 2 KiB → 1 MiB
  (`scrub.Truncate` callers across cowork / cline / claudecode /
  codex / copilotcli pick up the new ceiling automatically).
- Every adapter site that previously did `truncate(body, 4000)` for
  `ToolOutput` now routes through `contentcap.Cap`: cowork
  ...
  todo. The Claude Code `Stop` hook in `cmd/observer/hook.go`
  matches.
- Opencode's two 200-char `RawToolInput` caps (subtask /
  todo) lifted to the 1 MiB ceiling for consistency.
- The FTS5 indexer (`action_excerpts`) keeps its 2 KiB excerpt cap
  unchanged — search performance trumps copy-fidelity for that
  surface.

**Serving** (new endpoint + `/messages` elision):

- New `GET /api/action/<id>/full_text` returns the untruncated
  `raw_tool_input` + `raw_tool_output` for one action on demand.
  Returns 404 on unknown id, 400 on malformed path.
- `/api/session/<id>/messages` now caps inline `full_text` at 4 KiB
  (preview only), sets `full_text_elided: true` on rows that exceeded
  the cap, and surfaces `has_full_output: true` whenever
  `actions.raw_tool_output` is non-empty. Timeline payload stays
  bounded regardless of how large any single row's raw content grew
  post-migration.
- New `action_id` field on every `toolCallRow` so the frontend can
  call the on-demand endpoint without an extra round-trip.

**UI** (`SessionDetailPanel.tsx::ToolCallRowView`):

- `CopyOnClick` gains an optional `resolveValue` callback awaited on
  click; the primary + excerpt copy buttons now fetch the full body
  via `/api/action/<id>/full_text` and copy the resolved text when
  `full_text_elided` / `has_full_output` is set. Cached per-row so
  repeated clicks don't refetch.
- New **View** button appears on rows with elided content; opens a
  centered modal (z-[60], above the SessionDetailPanel slide-over)
  showing both `raw_tool_input` + `raw_tool_output` side-by-side
  with copy buttons inside. Escape + backdrop click close.

**Tests**:

- `internal/contentcap`: 6 cases covering passthrough, exact-fit,
  over-cap marker, zero/negative max default behavior.
- `internal/store`: `TestInsertActions_RawToolOutputLengthMerge`
  pins ON CONFLICT length-merge — re-ingest with longer body
  upgrades, shorter is ignored.
- `internal/intelligence/dashboard`: `TestAPIActionFullText` covers
  happy path (returns untruncated input + output), 404 on unknown
  id, 400 on malformed id, 404 on unsupported sub-resource.
- `internal/adapter/cursor`: existing 4000-cap test renamed +
  rewritten as `TestBuildEvent_BeforeReadFile_CapsAtContentcapDefault`
...
- `internal/adapter/codex/adapter.go` modern path
  (event_msg/token_count): emit `InputTokens = last.input_tokens -
  last.cached_input_tokens` (clamped at 0).
- `internal/adapter/codex/adapter.go` legacy path (top-level
  ...
  deltas when cached grew faster than gross).
- `internal/proxy/provider.go` OpenAI non-streaming response shape:
  `shape.InputTokens = max(0, PromptTokens - CachedTokens)`.
- `internal/proxy/streaming.go::applyOpenAIUsage`: same netting,
  with CacheReadTokens populated first so the netting step can read
  it off the result struct.
- `internal/adapter/copilotcli/log.go::emitTokenEvent` (Tier-1 OTel
  ...
  is Anthropic-net; updated to reflect the actual NET contract.
- `internal/adapter/copilotcli/events.go` session.shutdown
  modelMetrics handler: same netting (DB pattern matched the log
  path's gross shape across all 9 sampled rows: input always >=
  cache_read with 51-91% cache hit ratios).
- `internal/adapter/copilotcli/events.go` session.compaction_complete
  ...
  variants).
- `internal/adapter/cursor/adapter.go::BuildStopTokenEvent`: same
  netting. Cursor's stop-hook payload uses Anthropic-style field
  NAMES (`cache_read_tokens`) but populates `input_tokens` with the
  ...
  the turn). Resolves the cursor item flagged in commit 0a6a252.
- `internal/intelligence/cost/engine.go` TokenBundle.Input
...
- `cost.TestComputeBreakdown_OpenAIShape_NoDoubleBill` — pins the
  ...
  pre-fix wrong total as an anti-regression.
- `codex.TestParseTokenCount_NetsInputAgainstCached` — covers both
  parser paths (modern + legacy cumulative).
- Updated existing tests (4 proxy + 4 codex + 6 copilot-cli) that
...
|---|---|---|
| claude-code, cline, cowork, claudecode-via-proxy | Anthropic native (`cache_read_input_tokens`) | NET ✓ |
...
- The watcher derives the `.cursor` root from the agent-transcript
  path it's already parsing (so a Windows-side cursor read from WSL
  via /mnt/c works via `crossmount.TranslateForeignPath`), globs
  `chats/*/<conv>/store.db`, opens it read-only with a busy timeout,
  and scans blobs for the single `role:"system"` JSON blob (binary
  Merkle-node blobs are prefix-filtered out cheaply).
- Emits an `ActionSystemPrompt` row mirroring the codex shape: full
  scrubbed body in `raw_tool_input` (now uncapped at 1 MiB per the
  earlier full-text work), 200-char preview in `target`, `MessageID`
  `system:<hash>`, content-hashed `SourceEventID`
  `cursor-sysprompt:<conv>:<hash>` so re-scans dedup via the store's
  UNIQUE index.
- Emitted BEFORE the watcher's hook-deferral gate, because the live
...
- New Go protobuf walker (`storedb.go::pbWalk` / `parseSectionIndex`)
  decodes the section index defensively (malformed/truncated fields
  abort cleanly — the format is undocumented and may drift).
- The current budget is pinned via `meta.latestRootBlobId` (a session
  accumulates multiple root blobs as it grows; picking by scan order
  would be non-deterministic), with a marker-scan fallback.
- Emits one zero-cost informational row per non-empty scaffolding
  section (`prompt_section.tools` / `.rules` / `.skills` /
  `.subagents` / …), skipping `system_prompt` (own content row) and
  `conversation` / `summarized_conversation` (the captured turns).
  Token/char counts live in `target` + body; NO `token_usage` row is
  emitted — the tokens are already billed inside each turn's
  `input_tokens`, so a separate token row would double-count.
  Content-name-keyed `SourceEventID` dedups on re-scan.
- These rows use a NEW `prompt_context` action type (not
  `system_prompt`), so the dashboard renders them as "Prompt context"
  rather than mislabeling a "Rules" / "Tool definitions" row as
  "System prompt". `models.ActionPromptContext` added; frontend
  `actions.ts` registers both `system_prompt` ("System prompt") and
  `prompt_context` ("Prompt context") explicitly (previously
  `system_prompt` only got a humanized fallback label).
  ...
  *content* anywhere on disk (verified across `~/.cursor` — no
  ...
  section whose content is stored.
- Test: `TestParseSessionFile_EmitsPromptBudgetSections` (synthetic
...
`ToolCallRowView` now copies `tc.full_text || tc.target ||
tc.raw_tool_name` instead of the display-truncated `tc.target`. The
excerpt copy already preferred `tc.full_text`. Tooltip-show
condition also widened to trigger when `primaryFull.length >
primary.length` so the operator sees the longer text on hover even
...
**Unregister symmetry.** Three byte-exact `startsWith(h.Command,
binaryPath)` call sites remained in the uninstall path while v1.6.25
...
fires still exited 127 with stderr `/usr/bin/bash: line 1: D:programsxsuperbased-
observerbinobserver.exe: command not found` — note the stripped backslashes in
...
`modernc.org/sqlite` returned `disk I/O error (SQLITE_IOERR_SHORT_READ
4618)` on every read — every Windows Desktop session never landed. Verified
...
per-source mirror dir under `os.UserCacheDir()/superbased-observer/
opencode-mirror/<sha8>/`, open the mirror, return the mirrored path.
...
`docs/audits/opencode-audit-2026-05-21.md` documents the full audit: live
...
the partial-state pattern: `request_59a80a92...:usage` has `in=0,
out=177` and `request_dbbb2805...:usage` has `in=0, out=137` — the
...
`docs/audits/cursor-audit-2026-05-21.md`. Each was a silent dropout invisible
...
- `docs/audits/cursor-audit-2026-05-21.md` — full audit using the
...
audit at `docs/audits/antigravity-audit-2026-05-19.md`. The audit followed
...
[`docs/audits/pricing-audit-2026-05-19.md`](docs/audits/pricing-audit-2026-05-19.md)
...
[`docs/audits/copilot-vscode-audit-2026-05-19.md`](docs/audits/copilot-vscode-audit-2026-05-19.md)
...
`docs/audits/copilot-vscode-audit-2026-05-19.md` are intentionally not in
this patch:
...
[`docs/audits/cowork-audit-2026-05-19.md`](docs/audits/cowork-audit-2026-05-19.md)
...
The following findings from `docs/audits/cowork-audit-2026-05-19.md` are
...
documents:
[`docs/audits/codex-audit-2026-05-18.md`](docs/audits/codex-audit-2026-05-18.md),
[`docs/audits/opencode-audit-2026-05-19.md`](docs/audits/opencode-audit-2026-05-19.md),
and [`docs/audits/openclaw-audit-2026-05-19.md`](docs/audits/openclaw-audit-2026-05-19.md).
A separate implementation tracker
([`docs/plans/codex-audit-followthrough-plan-2026-05-19.md`](docs/plans/codex-audit-followthrough-plan-2026-05-19.md))
...
[`docs/audits/copilot-cli-events-schema-audit-2026-05-19.md`](docs/audits/copilot-cli-events-schema-audit-2026-05-19.md)
...
[`docs/handovers/next-session-handover-2026-05-19.md`](docs/handovers/next-session-handover-2026-05-19.md)).
...
[`docs/audits/backfill-flag-audit-2026-05-19.md`](docs/audits/backfill-flag-audit-2026-05-19.md):
...
| `docs/audits/backfill-flag-audit-2026-05-19.md` | NEW | 27-flag categorization + dry-run spec |
...
report: [`docs/audits/claude-code-audit-2026-05-18.md`](docs/audits/claude-code-audit-2026-05-18.md);
empirical scope: [`docs/audits/claude-code-audit-2026-05-18-scope.md`](docs/audits/claude-code-audit-2026-05-18-scope.md);
methodology follows
...
Documented in the prior audit `docs/audits/claudecode-bash-duration-audit-2026-05-15.md`:
...
[`docs/audits/copilot-cli-audit-2026-05-18.md`](docs/audits/copilot-cli-audit-2026-05-18.md);
...
**Audit doc (`docs/audits/copilot-cli-audit-2026-05-18.md`, 30 KB):** full
...
| `docs/audits/copilot-cli-audit-2026-05-18.md` | +662 NEW | audit report + V6k addendum |
...
`docs/plans/copilot-cli-adapter-plan.md` §11.3): WebSocket MITM proxy
...
- `docs/plans/copilot-cli-adapter-plan.md` (new, ~600 lines) — full
...
`docs/plans/cowork-adapter-plan.md`.
...
- `docs/plans/cowork-adapter-plan.md` (new) — comprehensive plan + checklists
...
- **`docs/audits/compression-audit.md`** — verifies which Compression
